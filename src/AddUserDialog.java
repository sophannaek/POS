...
public class AddUserDialog {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AddUserDialog window = new AddUserDialog();
	...
	private void initialize() {
		...
		JLabel lblAdminPanel = new JLabel("ADD USER");
	...
	protected void addUser() 
	{
		try
		{
			...
			if (userDAO.getAllUsers().contains(tempUser))
			{
				...
				lblUserExists.setText("User Already Exists");
				lblUserExists.setVisible(true);
			}
			else if(txtEmpNumber.getText().isEmpty() ||
					txtEmpUserName.getText().isEmpty() ||
			...
			else
			{
				...
				txtEmpNumber.requestFocus();
				chckbxIsadmin.setSelected(false);
				lblUserExists.setText("User Added");
				lblUserExists.setVisible(true);
			}
		}
		catch (Exception e)
